...
namespace WebHookExample.Properties
{
    ...
    public class WebhookController : Controller
    {
        ...
        {
              { 21476, "tWcjsKkX3aI9LsP7kJ4yCUMFNugrnq1ovRtzq59634707bbd" }
        };

        public WebhookController(ILogger<WebhookController> logger, DataBaseContext db/*, CreateMediaService  createMediaService*/, IMapper mapper)
        {
            _logger = logger;
            _db = db;
            _mapper = mapper;
                ...
                if (body.EventType == "message")
                {
                    ...
                    if (messageData != null || !string.IsNullOrEmpty(type))
                    {
                        ...
                        if (body.Data.media != null)
                        {
                            ...
                            obj.Media_mimeType = body.Data.media.mimetype.ToString();
                            obj.Media_filename = body.Data.media.filename != null ? body.Data.media.filename.ToString() : null;
                            obj.Media_data = body.Data.media.data.ToString();
                            obj.Media_filesize = body.Data.media.filesize;
                        ...
                        using (var transaction = await _db.Database.BeginTransactionAsync())
                        {
                            try
                            {
                                var result = _db.ReceivedMessages.Where(i => i.MessageId == messageId).ToList();
                                if (result.Count() == 0)
                                {
